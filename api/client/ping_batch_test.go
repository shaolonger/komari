...
func TestPingResultBatchIsAtomicContiguousAndIdempotent(t *testing.T) {
	...
	saves := 0
	accepted, err := acceptPingResultBatch(context.Background(), db, "node", 1, inputs, func(records []models.PingRecord, checkpoint models.PingResultSequence) error {
		saves++
	...
	accepted, err = acceptPingResultBatch(context.Background(), db, "node", 1, inputs, func([]models.PingRecord, models.PingResultSequence) error { saves++; return nil })
	if err != nil || !accepted.Duplicate || saves != 1 {
		t.Fatalf("duplicate=%#v saves=%d err=%v", accepted, saves, err)
	}
	accepted, err = acceptPingResultBatch(context.Background(), db, "node", 3, inputs, func([]models.PingRecord, models.PingResultSequence) error { return nil })
...
func TestPingResultBatchRejectsBoundsBeforeSave(t *testing.T) {
	...
	saved := false
	_, err := acceptPingResultBatch(context.Background(), db, "node", 1, make([]pingResultInput, maxPingResultBatch+1), func([]models.PingRecord, models.PingResultSequence) error { saved = true; return nil })
	if err == nil || saved {
